<template>
  ...
  <div v-if="isLoading" class="loading-spinner">
    <i class="fa-solid fa-spinner fa-spin"></i> Loading Training Modules ...
  </div>
  ...
  <div id="main-div" style="display: flex; margin-top: 30px">
    <div class="container mt-4">
      ...
      <div class="course-accordion">
        <div
          v-for="(course, courseIndex) in groupedCourses"
          :key="courseIndex"
          class="course-item"
        >
          <!-- Course Header -->
          ...
          <div v-if="expandedCourses[courseIndex]" class="course-content">
            <!-- Levels Accordion -->
            <div
              v-for="(level, levelIndex) in course.levels"
              :key="levelIndex"
              class="level-item"
            >
              <!-- Level Header -->
              <div
                ...
                class="level-header"
                @click="toggleLevel(courseIndex, levelIndex)"
              >
                ...
                <i
                  ...
                  class="fa"
                  :class="
                    expandedLevels[`${courseIndex}-${levelIndex}`]
                      ? 'fa-chevron-up'
                      : 'fa-chevron-down'
                  "
                ></i>
              ...
              <div
                v-if="expandedLevels[`${courseIndex}-${levelIndex}`]"
                class="level-content"
              >
                <div class="row">
                  <!-- Modules Grid -->
                  ...
                  >
                    ...
                    >
                      <p class="card-text">
                        <span
                          v-if="module.type.mandatory == 'true'"
                        ...
                        <span v-else>
                          ...
                          >
                          <span
                            v-if="module.type.campaign == 'true'"
                      ...
                      <div class="card-body d-flex flex-column flex-grow-1">
                        ...
                        <p
                          class="card-text truncate-text"
                          style="height: 30px"
                        ></p>
                        <p class="mb-1">
                          <i class="fa fa-play-circle"></i> Course :
                          {{ module.course_name }}
                        </p>
                        <p class="mb-1">
                        ...
                        <p class="mb-1">
                          <i class="fa fa-comment"></i> Statements :
                          {{
                            module.statements
                        ...
                        >
                          <!-- Show "Passed" in green if the status is "Passed" -->
                          <span
                          ...
                          <!-- Show "Failed" in red if the status is "Failed" -->
                          <span
                          ...
                          <!-- Show the button for other statuses, disabled if prerequisites not met -->
                          <button
                            v-else
      ...
      <div class="course-screen-modal-body">
        ...
        <div class="wizard-navigation">
          ...
          >
            ...
            <div class="step-type">
              {{ item.type === "question" ? "Question" : "Content" }}
            </div>
          </div>
        </div>
        ...
        <div class="content-area">
          <!-- Question Display -->
          <div
            v-if="currentItem && currentItem.type === 'question'"
          ...
          >
            ...
            <div
              ...
              v-html="currentItem.description_en"
            ></div>
            ...
            <div
              v-else-if="currentItem.question_type === 'multi_choice'"
              class="answer-options multi-choice"
            >
              <label
              ...
              >
                ...
                <span
                  v-if="isAnswerSubmitted && option.is_correct === 'true'"
                  class="correct-indicator"
                >
                  <i class="fa fa-check-circle"></i>
                </span>
          ...
          <div
            v-else-if="currentItem && currentItem.type === 'statement'"
            class="statement-container"
          >
            <div class="statement-header">
              <span class="statement-type-badge">Learning Content</span>
              <span class="statement-number"
                >Content {{ currentStatementNumber }} of
            ...
            <div
              ...
              v-html="currentItem.content_en"
            ></div>

            <!-- Media Content -->
            <div
              v-if="currentItem.additional_content_type === 'Embedded Image'"
              class="media-container"
            >
              <img
                :src="currentItem.file_en"
                alt="Statement Image"
                class="statement-media"
            ...
            <div
              v-else-if="currentItem.additional_content_type === 'Embedded PDF'"
              class="media-container"
            >
              <iframe
                :src="currentItem.file_en"
      ...
      <div class="completion-body">
        ...
        <div class="completion-message">
          <p v-if="hasValidQuestions && isPassed">
            You have successfully completed the "{{ selectedModule.name }}"
            module!
          </p>
          <p v-else-if="hasValidQuestions && !isPassed">
            You didn't reach the passing score for this module.
          </p>
          <p v-else>
            You have successfully completed the "{{ selectedModule.name }}"
            module!
...
export default {
  ...
  methods: {
    ...
    startModule(module) {
      ...
      Swal.fire({
        title: "Confirmation",
        text: "If you proceed to training and refresh the page, your score will be calculated and you will not be able to re-exercise. Are you sure you want to continue ?",
        icon: "warning",
        showCancelButton: true,
    ...
    canStartModule(module) {
      // Check if all modules in lower levels are completed
      const currentLevelOrder = module.level_order;
      const lowerLevelModules = this.moduleList.filter(
        (m) => m.level_order < currentLevelOrder
      );
      const areLowerLevelsCompleted = lowerLevelModules.every(
        (m) => m.status === "Passed" || m.status === "Failed"
      );

      // Check if all modules with lower order in the same level are completed
      const sameLevelLowerOrderModules = this.moduleList.filter(
        (m) =>
          m.level_id === module.level_id &&
          (m.order || 0) < (module.order || 0) &&
          m.id !== module.id
      );
      const areLowerOrderModulesCompleted = sameLevelLowerOrderModules.every(
        (m) => m.status === "Passed" || m.status === "Failed"
      );

      // Return true only if both conditions are met
      return areLowerLevelsCompleted && areLowerOrderModulesCompleted;
    },
    ...
    async saveModuleResult(status) {
      try {
        ...
        // Save user answers
        // await this.UserLmsTrainingModuleApi.saveUserAnswers({
        //   module_result_id: moduleResultResponse.id || moduleResultResponse._id,
...
async fetchModules() {
  ...
  try {
    ...
    const [allCourses, existingModules, moduleResults] = await Promise.all([
      this.api.getAll({ with: "levels" }),
      this.UserLmsTrainingModuleApi.getModules(),
      this.LmsUserAnswerApi.getAll({ filter: `user_id|${userId}` }),
    ]);
    ...
    // Filter courses based on user assignment, mandatory course, mandatory level, or optional course/level
    const userCourses = allCourses.filter((course) => {
      ...
      const hasMandatoryLevel =
        course.levels?.some(
          (level) =>
            level.levelType?.mandatory === "true" &&
            ((level.user_ids?.includes(userId) && level.user_ids[0] !== null) ||
      ...
      const hasOptionalLevel =
        course.levels?.some(
          (level) => level.levelType?.optional === "true"
        ) || false;

      return (
        (isCourseAssignedToUser &&
          (isCourseMandatory || hasMandatoryLevel)) ||
        isCourseOptional ||
        hasOptionalLevel
      );
    });

    console.log("User Courses:", JSON.stringify(userCourses, null, 2));

    // Create a Map for course lookup
    const courseMap = new Map(
      userCourses.map((course) => [course.id, course])
    );

    // Process modules for filtered courses
    ...
    for (const course of userCourses) {
      ...
      console.log(
        `Course ID: ${course.id}, Name: ${course.name}, Levels:`,
        course.levels
      );

      if (course.levels?.length) {
        for (const level of course.levels) {
          if (!level.lms_module_ids?.length) {
            console.log(
              `Skipping Level ID: ${level.id}, Title: ${level.title} (No valid lms_module_ids)`
            );
            continue;
          }
          ...
          const hasLevelType = isLevelMandatory || isLevelOptional || level.levelType?.campaign === "true";
          ...
          if (hasLevelType) {
            ...
            shouldIncludeLevel = isLevelOptional || (isLevelMandatory && isLevelAssignedToUser);
          } else {
            // Fall back to course type logic
            shouldIncludeLevel = isCourseOptional || (isCourseMandatory && isCourseAssignedToUser);
          }
          ...
          // console.log(`Processing Level ID: ${level.id}, Title: ${level.title}, Course ID: ${course.id}`);
          const modules = await Promise.all(
            level.lms_module_ids.map(async (moduleId) => {
              const module = await this.moduleApi.show(moduleId);
              let moduleType = {
                mandatory: "false",
                optional: "false",
                campaign: "false",
          ...
          mandatoryModules.push(...modules);
        }
      }
    }

    // Combine modules, removing duplicates
    const allModules = [
      ...existingModules,
      ...mandatoryModules.filter(
        (mandatoryModule) =>
          !existingModules.some(
            (existingModule) => existingModule.id === mandatoryModule.id
          )
      ),
    ];

    // Create a Map for level lookup
    const levelMap = new Map();
    userCourses.forEach((course) => {
      course.levels?.forEach((level) => levelMap.set(level.id, level));
    });

    // Process modules with course and level details concurrently
    this.moduleList = await Promise.all(
      allModules.map(async (module) => {
        const course =
          courseMap.get(module.course_id) ||
          (await this.api.show(module.course_id));
        const level = module.level_id
          ? levelMap.get(module.level_id) ||
            (await this.levelApi.show(module.level_id))
          : null;
...
.course-info {
  display: flex;
  flex-direction: column;
  gap: 5px; /* Space between course name and description */
}

.course-info h4 {
  margin: 0; /* Remove default margin for consistent spacing */
  color: white; /* Ensure course name is visible on dark background */
}

.course-info small {
  color: aliceblue; /* Keep description color as specified */
  font-size: 11px; /* Optional: adjust font size for better hierarchy */
}
