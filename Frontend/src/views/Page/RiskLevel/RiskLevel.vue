<template>
  ...
  <main-page ref="page" :isFlipped="isFlipped" :mainPage="'Configuration '" :mainSubPage="'Risk'"
    :subPage="'Risk Level'" :titlePage="'Risk Level'" v-permission:show>
...
export default {
  ...
  created() {
    ...
    this.tableColumns = [
      { title: this.$t("risklevel.name"), data: "name", defaultContent: "N/A" },
      { title: this.$t("risklevel.from"), data: "from", defaultContent: "N/A" },
      ];
    ...
    this.fromFields = [
      {
        name: "name",
        label: this.$t("risklevel.name"),
        type: "text", // Text input field
        rules: "required", // Validation rule: required field
        description: "Please enter the level name.",
        col: 4, // Column size in the form layout
      },
      {
        name: "from",
        label: this.$t("risklevel.from"),
        type: "number",
        rules: "required", // Validation rule: required and valid email format
        description: "Please enter a value between 1 and 25.",
        col: 4,
      },
      {
        name: "color",
        label: this.$t("risklevel.color"),
        type: "color", // Text input field for email
        rules: "required", // Validation rule: required and valid email format
