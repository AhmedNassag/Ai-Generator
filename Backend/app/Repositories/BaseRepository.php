...
class BaseRepository
{
    ...
    public function get(Request $request, $with = [], $withCount = [])
    {
        ...
        return $this->dataTable($query, $request, [], $with);
    }
     ...
     * @return \Illuminate\Database\Eloquent\Collection Paginated collection of models.
     */
    public function dataTable($query, $request, $columns = [], $with = [], $more = "", $moreData = [])
    {
        ...
        // Apply "where with relation" filter if present
        if ($request->filterWhereRelation) {

            foreach ($request->filterWhereRelation as $whereRelation) {
                $filter = explode("@", $whereRelation);
                $query->whereHas($filter[0], function ($subQuery) use ($filter) {
                    $this->where($subQuery, $filter[1]);
                });
            }
        }
        ...
        //     $query->where(function ($query) use ($request, $columns, $more, $moreData, $with) {
        //         if ($more != "") {
        ...
        //                     $relatedName = $relationArr[0];
        //                     $relatedColumns = explode(",", $relationArr[1]);
        //                 }

        //                 $query->orWhereHas($relatedName, function ($subQuery) use ($request, $relatedColumns) {
        //                     $subQuery->where(function ($subQuery) use ($request, $relatedColumns) {
        //                         // Get the fillable columns of the related model
        //                         if (count($relatedColumns) > 0) {
        //                             $relatedModel = $subQuery->getModel();
        //                             $relatedColumns = $relatedModel->getFillable();
        //                         }


        //                         // Search in each fillable column of the related model
        //                         foreach ($relatedColumns as $relatedColumn) {
        //                             $subQuery->orWhere($relatedColumn, 'LIKE', '%' . $request->search . '%');
        //                         }
        //                     });
        //                 });
        //             }
        ...
        if ($request->search && ($request->search != "")) {
            ...
            $query->where(function ($query) use ($request, $columns, $more, $moreData, $with) {
                ...
                if (!empty($more) && !empty($moreData)) {
                    $query->orWhereIn($more, $moreData);
                }

                // Search in main model columns
                foreach ($columns as $column) {
                    $query->orWhere($column, 'LIKE', '%' . $request->search . '%');
                }
                ...
                if (!empty($with)) {
                    foreach ($with as $relation) {
                        ...
                        // Check if specific columns are defined for relation
                        if (str_contains($relation, ":")) {
                            [$relatedName, $relatedColumnsStr] = explode(":", $relation);
                            $relatedColumns = explode(",", $relatedColumnsStr);
                        }

                        $query->orWhereHas($relatedName, function ($subQuery) use ($request, $relatedColumns) {
                            // Fetch model columns dynamically if not provided
                            if (empty($relatedColumns)) {
                                $relatedColumns = $subQuery->getModel()->getFillable();
                            }

                            // Ensure related model has searchable columns
                            if (!empty($relatedColumns)) {
                                $subQuery->where(function ($subQuery) use ($request, $relatedColumns) {
                                    foreach ($relatedColumns as $relatedColumn) {
                                        $subQuery->orWhere($relatedColumn, 'LIKE', '%' . $request->search . '%');
     ...
     *
     * This method retrieves the first available record in the model. If a record
     * is found, it updates the record with the provided data. If no record is found,
     * it creates a new record using the specified data.
     *
     * @param array $data The data to update the existing record or create a new record.
    ...
    public function updateOrCreate( $data)
    {
        ...
        if (!empty($data['id'])) {
            ...
            if ($item) {
                $item->update($data); // Update the existing record
            } else {
